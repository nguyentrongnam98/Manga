...
class OpenMovie extends Component {
  ...
  getMovieHandler = (movieId) => {
    ...
    axios
      ...
      axios.get('https://api.nytimes.com/svc/movies/v2//reviews/search.json?query=deadpool&api-key=ePRCx9IlArKkt2NPpxLlhrFM8N6WN9Gx')
      .then(response => {
        const reviews = response.data.results;
        for (let i=0; i < reviews.length; i++) {
          reviewArr.push({
             author: reviews[i].byline,
             content: reviews[i].summary_short,
             url: reviews[i].link.url,
             type: 'nytimes', 
             pic: reviews[i].multimedia.src,
             title: reviews[i].headline
          });
        }
      }).catch(error => {
  ...
  render() {
    ...
    if (this.state.reviewResults) {
      ...
      reviews = this.state.reviewResults.map(review => {
        return (
          <Review
            key={review.id}
